...
const ViewAll = () => {
    ...
    const [sortConditionCourse, setSortConditionCourse] = useState({ 'field': "", "direction": "" })
    const [sortConditionTeacher, setSortConditionTeacher] = useState({ 'field': "", "direction": "" })
    const [sortConditionCenter, setSortConditionCenter] = useState({ 'field': "", "direction": "" })
    ...
    const handleSelectTag = (tag) => {
        if (menuIndex === 1) {
            ...
            if (!selectedTagsCenter.find((selected) => selected.idTag === tag.idTag)) {
    ...
    const handleRemoveTag = (idTag) => {
        ...
        else setSelectedTagsCenter(selectedTagsCenter.filter((tag) => tag.idTag !== idTag));
    ...
    const handleFieldChange = (e) => {
        if (menuIndex === 1)
            ...
            setSortConditionTeacher({ ...sortConditionTeacher, field: e.target.value });
        else setSortConditionCenter({ ...sortConditionCenter, field: e.target.value })
    };

    const handleDirectionChange = (e) => {
        if (menuIndex === 1)
            setSortConditionCourse({ ...sortConditionCourse, direction: e.target.value });
        else if (menuIndex === 2)
            setSortConditionTeacher({ ...sortConditionTeacher, direction: e.target.value });
        else setSortConditionCenter({ ...sortConditionCenter, direction: e.target.value })
    };
        ...
        if (courseList && menuIndex === 1) {
            const filteredCourse = courseList
                .filter(course =>
                    ...
                    (selectedTags.length === 0 || course.tags.some(tag => selectedTags.some(selected => selected.tagName === tag))) &&
                    ((!parseInt(startPrice) && !parseInt(endPrice)) || (course.price >= parseInt(startPrice) && course.price <= parseInt(endPrice))) &&
                    (courseType === "All" || (courseType === "Limit" && course.isLimitedTime === 1) || (courseType === "Unlimited" && course.isLimitedTime === 0)) &&
                    ((courseType !== "limit" && courseType !== "all") ||
                        (
                            (!startDate || new Date(course.courseStartDate) >= new Date(startDate)) &&
                            (!endDate || new Date(course.courseEndDate) <= new Date(endDate)) &&
                            (!startRegistration || new Date(course.registStartDate) >= new Date(startRegistration)) &&
                            (!endRegistration || new Date(course.registEndDate) <= new Date(endRegistration))
                        )
                    )
                )
                .sort((a, b) => {
                    ...
                    const aValue = field.includes("Date") ? new Date(a[field]).getTime() : a[field];
                    const bValue = field.includes("Date") ? new Date(b[field]).getTime() : b[field];
            ...
            const filteredTeacher = teacherList
                .sort((a, b) => {
                    ...
                    const aValue = field === "courseCount"
                        ? (a.courseCount || a.coursesCount)
                        : (field === "name" ? (a.name || a.fullName || "")
                            : (field === "teachingMajor" ? (a.teachingMajor || "") : a[field]));

                    const bValue = field === "courseCount"
                        ? (b.courseCount || b.coursesCount)
                        : (field === "name" ? (b.name || b.fullName || "")
                            : (field === "teachingMajor" ? (b.teachingMajor || "") : b[field]));
                ...
                .filter(teacher =>
                    (teacher.name?.toLowerCase().includes(content?.toLocaleLowerCase())) ||
                    (teacher.fullName?.toLowerCase().includes(content?.toLocaleLowerCase())) ||
                    (teacher.teachingMajor?.toLowerCase().includes(content?.toLocaleLowerCase())) ||
            ...
            const filteredCenter = centerList
                .filter(center =>
                    ((selectedTagsCenter.length === 0 || center.listTagCourses.some(tag => selectedTagsCenter.some(selected => selected.tagName === tag.tagName)))) &&
                    ((!parseInt(minCourseNumber) && !parseInt(maxCourseNumber)) || (center.coursesCount >= parseInt(minCourseNumber) && center.coursesCount <= parseInt(maxCourseNumber)))
                )
                .sort((a, b) => {
                    // Ensure sortConditionCourse has valid values
                    if (!sortConditionTeacher.field || !sortConditionTeacher.direction) {
                ...
                .filter(center =>
                    (center.centerName?.toLocaleLowerCase().includes(content?.toLocaleLowerCase())) ||
                    (center.description?.toLocaleLowerCase().includes(content?.toLocaleLowerCase())) ||
    ...
    if (menuIndex === 1) {
        currentItemsPage.coursePage = currentCourses.slice(currentPage.coursePage * itemsPerPage - itemsPerPage, currentPage.coursePage * itemsPerPage);
    } else if (menuIndex === 2) {
        currentItemsPage.teacherPage = currentTeachers.slice(currentPage.teacherPage * itemsPerPage - itemsPerPage, currentPage.teacherPage * itemsPerPage);
    } else {
            ...
            <div className="filter-search-section">
                ...
                <div className="search-container">
                    <input
                        ...
                        placeholder="Search"
                        value={menuIndex === 1 ? searchCourse : (menuIndex === 2 ? searchTeacher : searchCenter)}
                        onChange={(event) => {
                            if (menuIndex === 1)
                    ...
                    {menuIndex
                        ...
                        (
                            <div className="main-box">
                                    ...
                                    <div className="tag-field">
                                        ...
                                        {selectedTags && selectedTags !== 0 && (
                                            <div className="tag-container">
                                                {selectedTags.map((tag) => (
                                                    <div
                                                        key={tag.idTag}
                                                        className="tag-showed"
                                                    >
                                                        <span>{tag.tagName}</span>
                                                        <button
                                                            onClick={() => handleRemoveTag(tag.idTag)}><LuX /></button>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                        ...
                                        >
                                            {tagList && tagList.length !== 0 &&
                                                tagList
                                                    .filter((tag) => tag.tagName.toLowerCase().includes(tagName.toLowerCase())) // Filter tags based on input
                                                    .map((tag) => (
                                                        <button
                                                            key={tag.idTag}
                                ...
                                {courseType && (courseType === "All" || courseType === "Limited") && (
                                    <>
                                        <div className="field">
                                            ...
                                            <div className="input-number-container">
                                                <input
                                                    ...
                                                    onChange={(event) => setStartRegistration(event.target.value)}
                                                />
                                            </div>
                                            <LuArrowRight />
                                            <div className="input-number-container">
                                                <input
                                                    ...
                                                    onChange={(event) => setEndRegistration(event.target.value)}
                                                />
                                            </div>
                                        </div>
                                ...
                                <div className="field tag">
                                    ...
                                    <div className="tag-field">
                                        ...
                                        >
                                            {tagList && tagList.length !== 0 &&
                                                tagList
                                                    .filter((tag) => tag.tagName.toLowerCase().includes(tagNameCenter.toLowerCase())) // Filter tags based on input
                                                    .map((tag) => (
                                                        <button
                                                            key={tag.idTag}
                                                            onClick={() => handleSelectTag(tag)}
