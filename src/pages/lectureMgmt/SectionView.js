...
import { LuCheck, LuCheckCircle, LuPenLine, LuPlus, LuTrash2, LuX } from "react-icons/lu";
import "../../assets/css/LectureView.css";
import { Role } from "../../constants/constants";
const SectionView = (props) => {
    const { idUser, idLecture, idSection, sectionList, setIdLecture, setIdSection } = props;
        ...
        <div className="section-view-container slide-to-bottom">
            <div className="section-header">
                ...
                <span className="header-info">
                    {`${sectionList.sectionCount} ${sectionList.sectionCount > 1 ? "sections" : "section"}`} -&nbsp;
                    {`${sectionList.lectureCount} ${sectionList.lectureCount > 1 ? "lectures" : "lecture"}`}
                </span>
            </div>
            <div className="section-display">
                {sectionList.sectionStructures && sectionList.sectionStructures.length > 0
                    && sectionList.sectionStructures.map((section, index) => (
                        <div key={index} className="each-section">
                            <div
                                className={`each-section-header ${isShowed[section.idSection] ? "active" : ""}`}
                                onClick={() => {
                                    setIsShowed({ ...isShowed, [section.idSection]: !isShowed[section.idSection] });
                                }}
                            >
                                {!isEdit[index] ?
                                    (<>
                                        <span className="each-section-name">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setIsEdit({ ...Object.keys(isEdit).reduce((acc, key) => ({ ...acc, [key]: false }), {}), [section.idSection]: true })
                                                }}>
                                                <LuPenLine />
                                            </button>
                                            {section.sectionName}
                                        ...
                                        <div className="each-section-info">
                                            <span className="lecture-number">
                                                {`${section.lectureCount} ${section.lectureCount > 1 ? "lectures" : "lecture"}`}
                                            </span>
                                            <button
                                                onClick={() => {
                                                    setIsShowed({ ...isShowed, [section.idSection]: !isShowed[section.idSection] });
                                            ...
                                            >
                                                {isShowed[section.idSection] ? (<IoIosArrowUp />) : (<IoIosArrowDown />)}
                                            </button>
                                        </div>
                                    </>)
                                    :
                            ...
                            {section.lectureStructures && section.lectureStructures.length > 0
                                && section.lectureStructures.map((lecture) => (
                                    <div key={lecture.idLecture} className={`section-lectures ${isShowed[section.idSection] ? "" : "deactive"}`}>
                                        <div
                                            className="lecture-item"
                                            onClick={() => setIdLecture(lecture.idLecture)}
                                        >
                                            <div className="lecture-info">
                                                <span className={`lecture-title ${idLecture === lecture.idLecture ? "active" : ""}`}>
